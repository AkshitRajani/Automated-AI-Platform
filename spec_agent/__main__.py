...
CLI: run the requirement agent on one app's analyzer output.
    ...
    PYTHONPATH=/path/to/2026/implementation \\
      python -m spec_agent ANALYZER_OUTPUT.yaml --app-id APP \\
...
def main(argv=None) -> int:
    p = argparse.ArgumentParser(prog="spec_agent", description=__doc__)
    ...
    p.add_argument("--worksheet", default="", help="human name-sheet YAML (doc 10) applied over the quad file's bindings")
    args = p.parse_args(argv)

    kwargs = dict(app_id=args.app_id, analyzer_output=args.analyzer_output,
                  codebase=args.codebase, worksheet=args.worksheet)
    if args.workspace:
        kwargs["workspace_dir"] = args.workspace
    task = RequirementTask(**kwargs)

    print(f"spec_agent: app={task.app_id} analyzer={task.analyzer_output}",
          file=sys.stderr)
    print(f"  workspace: {task.workspace_dir}", file=sys.stderr)

    # Imported here so `--help` doesn't require the Strands runtime.
    from spec_agent.boundary import run_with_boundary
