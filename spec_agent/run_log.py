"""
Run log — capture the agent's full activity to a plain-text file (verbatim from
the coding agent). Whoever runs the agent gets a complete, human-readable trace of
what it did and why: every reasoning line and every tool call with its arguments,
in order. Wired on by default in the boundary; the file lands next to the output
as ``<workspace>/agent_log.txt``.
"""
...
def _fmt_tool_args(inp) -> str:
    ...
    for k in ("path", "file", "file_path", "query", "name", "pattern", "q",
              "command", "cmd", "predicate", "app_id", "unit", "glob", "start", "kind"):
        v = inp.get(k)
        if v:
