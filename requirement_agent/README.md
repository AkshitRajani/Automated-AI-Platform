...
nine-section, nine-section spec your Automated AI Platform viewer produces — by reading the analyzer's
...
Place the `requirement_agent/` folder inside a directory on your `PYTHONPATH`, then:

```bash
pip install -r requirement_agent/requirements.txt
```
...
```yaml
metadata:
  app_id: DCFO                       # optional; --app-id overrides
entities:
...
From the directory that contains the `requirement_agent/` package:

```bash
PYTHONPATH=. python -m requirement_agent  <analyzer_output.yaml>  --app-id <APP_ID>  [options]
```

| Argument | Required | Meaning |
|---|---|---|
| `analyzer_output.yaml` | yes | path to the analyzer quad file |
| `--app-id` | yes | the application id / KB scope (e.g. `DCFO`) |
...
```bash
PYTHONPATH=. python -m requirement_agent  ./quads/dcfo.yaml \
  --app-id DCFO \
  --codebase ./src/dcfo.zip \
  --workspace ./out/dcfo
```
...
│   ├── <unit-slug>.json
│   └── ...
├── requirements_md/              # HUMAN-READABLE: the same docs as nine-section markdown
│   ├── <unit-slug>.md
│   └── ...
...
```bash
PYTHONPATH=. python -m pytest requirement_agent/tests/ -q
```
...
| `render.py` | turns a document's JSON into nine-section markdown |
