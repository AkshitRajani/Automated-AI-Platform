# Grace path no-fee

**Unit:** `WF-no-fee`
**Domain:** DCFO late fee and payment operations

## Overview
Specifies behavioural expectations for grace path no-fee within DCFO late-fee operations.

## User Stories
### US-01
As a customer within grace I should not be charged a late fee.

## Consolidated Requirements
- The system shall leave the balance unchanged when grace validation fails the overdue rule

## Acceptance Criteria
- AC-1: Verify that the system leave the balance unchanged when grace validation fails the overdue rule

## Data & Rules
Key fields: account_id, payment_due_date, grace_days, fee_rate_pct, outstanding_balance, waiver_reason, notification_status.

## Non-Functional
Assessment and waiver actions must be auditable. Notification retries must be idempotent with respect to fee charges.
